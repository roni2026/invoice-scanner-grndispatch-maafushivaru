...
class MaafushivaruHub(tk.Tk, OCRWorkerMixin):
    ...
    def _load_config(self):
        ...
        return cfg
        s.setdefault("ocr_word_correction_enabled", False)
        s.setdefault("smart_cross_match_enabled", False)
    ...
    def _build_settings_tab(self):
        ...
        tk.Label(prow, text="Provider:", bg=PANEL, fg=MUTED,
                 font=("Segoe UI", 9, "bold"), width=16, anchor="w").pack(side=tk.LEFT)
        self._ai_provider_var = tk.StringVar(value=ai_cfg.get("provider", "anthropic"))
        ...
        tk.Label(mrow, text="Model:", bg=PANEL, fg=MUTED,
                 font=("Segoe UI", 9, "bold"), width=16, anchor="w").pack(side=tk.LEFT)
        self._ai_model_var = tk.StringVar(value=ai_cfg.get("model", "claude-haiku-4-5-20251001"))
        self._ai_model_entry = tk.Entry(
            mrow, textvariable=self._ai_model_var,
        ...
        tk.Label(krow, text="API Key:", bg=PANEL, fg=MUTED,
                 font=("Segoe UI", 9, "bold"), width=16, anchor="w").pack(side=tk.LEFT)
        self._ai_key_var = tk.StringVar(value=ai_cfg.get("api_key", ""))
        tk.Entry(
            krow, textvariable=self._ai_key_var,
            bg=PANEL2, fg=TEXT, insertbackground=TEXT,
            relief="flat", font=("Segoe UI", 10), bd=0,
            highlightthickness=1, highlightbackground=PANEL3, highlightcolor=ACCENT,
            show="*", width=52,
    ...
    def _test_ai_connection(self):
        ...
        def _run():
            # Save current UI values to config first
            ...
            self._ai_matcher.config = self.cfg
            ...
            dot = {"connected": "●", "disconnected": "✗", "low_credit": "⚠", "offline": "○"}.get(status_code, "○")
